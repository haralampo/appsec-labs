...
## Exploitation
In order for a UNION query to succeed, the number of columns selected from the new table had to match the number of columns selected from the initial table. This second value was currently unknown, so I attempted to UNION an increasing amount of `NULL` columns and saw if the results had any giveaways. I used `NULL` because the columns being joined also had to match in data type, and `NULL` was convertible to every common data type.
...
### Second method
Another method involved trying to order the query result by incrementing column indexes, such as `' ORDER BY 1--`. The idea was that, if I got an error, I was trying to order by a column that did not exist, and so I would know that the query returned one less than that number of columns.
